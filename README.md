...
このリポジトリは、長期インターンで自然言語処理を体系的に学んだ成果をまとめたものです。
基礎的な文字列処理から、形態素・構文解析、機械学習、ニューラルネットワーク、
事前学習モデル、生成AI、機械翻訳までをPythonで実装しています。

## 取り組んだ内容

| 章 | 主な内容 |
|---|---|
| 1〜2 | 文字列処理、UNIXコマンド相当のデータ加工 |
| 3〜4 | Wikipedia記事の抽出、正規表現、MeCab・CaboChaによる日本語解析 |
| 5 | Gemini APIを使ったプロンプト設計、評価、出力の頑健性検証 |
| 6 | word2vec、類似度・アナロジー評価、クラスタリング、可視化 |
| 7 | 特徴量設計、ロジスティック回帰、評価指標、正則化 |
| 8 | 単語埋め込みとPyTorchによるニューラル分類器 |
| 9 | BERTの推論・ファインチューニング |
| 10 | GPT-2による生成・分類・SFT・DPO |
| 2020年版 第10章 | Seq2Seq、Attention、beam search、SentencePiece、ドメイン適応、推論API |

### コードを見るときの入口

- [chapter_4/wiki_corpus.py](chapter_4/wiki_corpus.py): Wikipediaコーパスの読み込みと日本語テキストの前処理
- [chapter_7/67.py](chapter_7/67.py): 感情分析モデルの評価指標
- [chapter_10/99.py](chapter_10/99.py): DPOによる言語モデルの選好学習
- [chapter_10_2020/95_2020ver.py](chapter_10_2020/95_2020ver.py): SentencePieceを使った機械翻訳モデル
- [scripts/prepare_data.py](scripts/prepare_data.py): 配布データのSHA-256検証と再現可能な配置

公開版には第三者データ、モデル重み、生ログ、認証情報を含めず、ソースコードと
匿名化した再利用可能な結果だけを収録しています。
